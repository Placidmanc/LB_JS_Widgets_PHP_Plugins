<?php
function acf_locations_probate_shortcode($atts)
{
  global $wp;
  ...
  if ($location_id) {
    ...
    if ($page === 'probate') {
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              $query->the_post();
              if (have_rows('probate_hero', $location_id)) {
                while (have_rows('probate_hero', $location_id)) {
                  the_row();
                  if ($field === 'title') {
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('probate_team', $location_id)) {
              while (have_rows('probate_team', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('probate_how_to_get', $location_id)) {
              while (have_rows('probate_how_to_get', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('probate_our_approach', $location_id)) {
              while (have_rows('probate_our_approach', $location_id)) {
                the_row();
                $copy = get_sub_field('copy', $location_id, true);
                $output .= '<p>' . $copy . '</p>';
              }
    ...
    if ($page === 'probate') {
      ...
      } else if ($section === 'team' || $section === 'how_does' || $section === 'approach') {
        $output .= $default;
      }
      return $output;
    }
...
add_shortcode('acf_locations_probate', 'acf_locations_probate_shortcode');
