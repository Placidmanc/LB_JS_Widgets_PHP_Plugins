<?php
function acf_locations_divorce_shortcode($atts)
{
  global $wp;
  ...
  if ($location_id) {
    ...
    if ($page === 'divorce') {
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              $query->the_post();
              if (have_rows('divorce_hero', $location_id)) {
                while (have_rows('divorce_hero', $location_id)) {
                  the_row();
                  if ($field === 'title') {
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('divorce_team', $location_id)) {
              while (have_rows('divorce_team', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('divorce_how_does', $location_id)) {
              while (have_rows('divorce_how_does', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('divorce_help', $location_id)) {
              while (have_rows('divorce_help', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
                $output .= '<h2 class="section-hdr-thin-bold">' . $main_title_thin . '<br><span style="font-weight:900;">' . $main_title_bold . '</span></h2>';
    ...
    if ($page === 'divorce') {
      ...
      } else if ($section === 'team' || $section === 'how_does' || $section === 'help') {
        $output .= $default;
      }
      return $output;
    }
...
add_shortcode('acf_locations_divorce', 'acf_locations_divorce_shortcode');
