<?php
function acf_locations_remortgage_shortcode($atts)
{
  global $wp;
  ...
  if ($location_id) {
    ...
    if ($page === 'remortgage') {
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              $query->the_post();
              if (have_rows('remortgage_hero', $location_id)) {
                while (have_rows('remortgage_hero', $location_id)) {
                  the_row();
                  if ($field === 'title') {
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('remortgage_team', $location_id)) {
              while (have_rows('remortgage_team', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
      ...
      } else if ($section === 'how_can') {
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('remortgage_how_can', $location_id)) {
              while (have_rows('remortgage_how_can', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
                $output .= '<h2 class="section-hdr-thin-bold">' . $main_title_bold . '<br><span style="font-weight:900;">' . $main_title_thin . '</span></h2>';
              }
            }
          }
        }
    ...
    if ($page === 'remortgage') {
      ...
      } else if ($section === 'team' || $section === 'how_can') {
        $output .= $default;
      }
      return $output;
    }
...
add_shortcode('acf_locations_remortgage', 'acf_locations_remortgage_shortcode');
