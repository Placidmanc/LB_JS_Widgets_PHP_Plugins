<?php
function acf_locations_lpa_shortcode($atts)
{
  global $wp;
    ...
    if ($page === 'lpa') {
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              $query->the_post();
              if (have_rows('lpa_hero', $location_id)) {
                while (have_rows('lpa_hero', $location_id)) {
                  the_row();
                  if ($field === 'title') {
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('lpa_team', $location_id)) {
              while (have_rows('lpa_team', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('lpa_how_does', $location_id)) {
              while (have_rows('lpa_how_does', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
...
add_shortcode('acf_locations_lpa', 'acf_locations_lpa_shortcode');
