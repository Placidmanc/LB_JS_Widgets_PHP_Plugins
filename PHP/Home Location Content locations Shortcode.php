<?php
function acf_locations_home_shortcode($atts)
{
  global $wp;
      ...
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              ...
              if (have_rows('home_hero', $location_id)) {
                while (have_rows('home_hero', $location_id)) {
                  the_row();
                  ...
                  if ($field === 'title') {
                    ...
                    $output .= '<p class="elementor-heading-title">' . $main_title_thin . '<br><span style="font-weight:900;">' . $main_title_bold . '</span></p>';
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            ...
            if (have_rows('home_services', $location_id)) {
              while (have_rows('home_services', $location_id)) {
                the_row();
...
add_shortcode('acf_locations_home', 'acf_locations_home_shortcode');
