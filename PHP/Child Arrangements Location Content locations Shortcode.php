<?php
function acf_locations_child_shortcode($atts)
{
  global $wp;
      ...
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              ...
              if (have_rows('child_hero', $location_id)) {
                while (have_rows('child_hero', $location_id)) {
                  the_row();
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            ...
            if (have_rows('child_services', $location_id)) {
              while (have_rows('child_services', $location_id)) {
                the_row();
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('child_help', $location_id)) {
              while (have_rows('child_help', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
...
add_shortcode('acf_locations_child', 'acf_locations_child_shortcode');
