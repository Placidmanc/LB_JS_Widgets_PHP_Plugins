<?php
function acf_locations_buying_shortcode($atts)
{
  global $wp;
    ...
    if ($page === 'buying') {
      if ($section === 'hero') {
        if ($field === 'location') {
          ...
          if ($query->have_posts()) {
            while ($query->have_posts()) {
              $query->the_post();
              if (have_rows('buying_hero', $location_id)) {
                while (have_rows('buying_hero', $location_id)) {
                  the_row();
                  if ($field === 'title') {
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('buying_services', $location_id)) {
              while (have_rows('buying_services', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
                $output .= '<h2 class="section-hdr-bold-thin-purple-large">' . $main_title_bold . '<br><span style="font-weight:300;">' . $main_title_thin . '</span></h2>';
              }
            }
          }
        }
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('buying_team', $location_id)) {
              while (have_rows('buying_team', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
        ...
        if ($query->have_posts()) {
          while ($query->have_posts()) {
            $query->the_post();
            if (have_rows('buying_how_does', $location_id)) {
              while (have_rows('buying_how_does', $location_id)) {
                the_row();
                $main_title_bold = get_sub_field('main_title_bold', $location_id, true);
                $main_title_thin = get_sub_field('main_title_thin', $location_id, true);
...
add_shortcode('acf_locations_buying', 'acf_locations_buying_shortcode');
